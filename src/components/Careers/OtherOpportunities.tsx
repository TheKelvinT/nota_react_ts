    ...
    <section className=" flex flex-col items-center gap-y-8 max-w-[480px] mx-auto  py-32">
      <div className="flex flex-col text-sub space-y-8 w-11/12">
        <div className="space-y-3 text-center sm:text-start">
          <h5 className="font-gothic font-bold text-sm">{data && data?.lastSection.pageHeader}</h5>
          <DescContainer >
            <div className="text-[10px]">
            <PortableText value={data && data?.lastSection.description || []}    onMissingComponent={false}/>
            </div>
          </DescContainer>
        </div>
