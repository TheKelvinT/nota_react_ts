...
const JobApplication = ({ opening }: Props) => {
  ...
  const onFinish = async (values: any) => {
    ...
    const formattedAvailability = moment(values?.availability?.$d).format(
      "DD-MMMM-YYYY",
    );
          ...
          if (fileList && fileList.length > 0) {
            ...
            if (!isValidSize) {
              return Promise.reject(
                new Error("File size must be less than 20MB."),
              );
            }
          }
      ...
      <div>
        ...
        >
          <Form.Item
            ...
            label={<p className="font-gothic text-base">First Name</p>}
          >
          ...
          <Form.Item
            ...
            label={<p className="font-gothic text-base">Last Name</p>}
          >
            <Input className="bg-primary border border-main/20 text-xs rounded-none w-full text-black/70  py-2 font-inter " />
          </Form.Item>
          <Form.Item
            name="contact"
            rules={rules.contact}
            label={<p className="font-gothic text-base">Contact No</p>}
          >
            <Input className="bg-primary border border-main/20 text-xs rounded-none w-full text-black/70  py-2 font-inter " />
          </Form.Item>
          <Form.Item
            name="email"
            rules={rules.email}
            label={<p className="font-gothic text-base">Email</p>}
          >
            <Input className="bg-primary border border-main/20 text-xs rounded-none w-full text-black/70  py-2 font-inter " />
          </Form.Item>
          <Form.Item
            name="dob"
            rules={rules.date}
            label={<p className="font-gothic text-base">Date of Birth</p>}
          >
            <DatePicker
              name="dob"
              id="dob"
          ...
          <Form.Item
            ...
            label={<p className="font-gothic text-base">Nationality</p>}
          >
            <Select
              showSearch
          ...
          <Form.Item
            ...
            label={
              <p className="font-gothic text-base">
                Earliest Date Availability
              </p>
            }
          >
            <DatePicker
